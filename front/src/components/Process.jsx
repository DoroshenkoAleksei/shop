import React from "react";

export default function Order() {
  return (
    <>
    <h1 className="header">Our products</h1>
    <div className="row row-cols-1 row-cols-sm-2 row-cols-md-5 g-3 ">
      {/* {"{"}% for product in products %{"}"} */}
      <div className="col">
        <div className="card border-0">
          {/* <img
            className="img-fluid"
            alt="Responsive image"
            src="{{ product.image.url }}"
          /> */}
          <div className="card-body px-0">
            <p className="card-text">
              <a
                className="text-dark text-decoration-none"
                href="{{ product.get_absolute_url }}"
              >
                {/* {"{"}
                {"{"} product.title|slice:":50" {"}"}
                {"}"}... */}
              </a>
            </p>
            {/* <div className="fw-bold">
              ${"{"}
              {"{"}product.price{"}"}
              {"}"}
            </div> */}
          </div>
        </div>
      </div>
      {/* {"{"}% endfor %{"}"} */}
    </div>
    <br />
    <br />
    {/* How to place an order BLOCK
     */}
    <div style={{ display: "flex", alignItems: "center", marginBottom: "1em" }}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={48}
        height={48}
        viewBox="0 0 48 48"
        fill="none"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M41.46 14.24L41.18 13.74C40.8188 13.1354 40.3094 12.6329 39.7 12.28L26.28 4.54C25.6724 4.1875 24.9826 4.00124 24.28 4H23.7C22.9974 4.00124 22.3076 4.1875 21.7 4.54L8.28 12.3C7.67394 12.6505 7.17052 13.1539 6.82 13.76L6.54 14.26C6.1875 14.8677 6.00124 15.5575 6 16.26V31.76C6.00124 32.4626 6.1875 33.1524 6.54 33.76L6.82 34.26C7.17958 34.859 7.68098 35.3604 8.28 35.72L21.72 43.46C22.3246 43.8198 23.0164 44.0066 23.72 44H24.28C24.9826 43.9988 25.6724 43.8126 26.28 43.46L39.7 35.7C40.312 35.3574 40.8174 34.852 41.16 34.24L41.46 33.74C41.8082 33.1306 41.9942 32.442 42 31.74V16.24C41.9988 15.5375 41.8126 14.8477 41.46 14.24ZM23.7 8H24.28L36 14.76L24 21.68L12 14.76L23.7 8ZM26 39L37.7 32.24L38 31.74V18.22L26 25.16V39Z"
          fill="#FF406E"
        />
      </svg>
      <div style={{ marginLeft: "1em"}}>
        <h3>Place your order</h3>
        <p>Place your order through our website</p>
      </div>
    </div>
    <div
      style={{ width: 2, height: 72, backgroundColor: "#FF406E", marginLeft: 25 }}
    />
    <div style={{ display: "flex", alignItems: "center", marginBottom: "1em" }}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={48}
        height={48}
        viewBox="0 0 48 48"
        fill="none"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M41.46 14.24L41.18 13.74C40.8188 13.1354 40.3094 12.6329 39.7 12.28L26.28 4.54C25.6724 4.1875 24.9826 4.00124 24.28 4H23.7C22.9974 4.00124 22.3076 4.1875 21.7 4.54L8.28 12.3C7.67394 12.6505 7.17052 13.1539 6.82 13.76L6.54 14.26C6.1875 14.8677 6.00124 15.5575 6 16.26V31.76C6.00124 32.4626 6.1875 33.1524 6.54 33.76L6.82 34.26C7.17958 34.859 7.68098 35.3604 8.28 35.72L21.72 43.46C22.3246 43.8198 23.0164 44.0066 23.72 44H24.28C24.9826 43.9988 25.6724 43.8126 26.28 43.46L39.7 35.7C40.312 35.3574 40.8174 34.852 41.16 34.24L41.46 33.74C41.8082 33.1306 41.9942 32.442 42 31.74V16.24C41.9988 15.5375 41.8126 14.8477 41.46 14.24ZM23.7 8H24.28L36 14.76L24 21.68L12 14.76L23.7 8ZM26 39L37.7 32.24L38 31.74V18.22L26 25.16V39Z"
          fill="#FF406E"
        />
      </svg>
      <div style={{ marginLeft: "1em" }}>
        <h3>Wait for the order confirmation </h3>
        <p>
          Wait for our agent to contact you to confirm order details
          <br /> and final costs of the package
        </p>
      </div>
    </div>
    <div
      style={{ width: 2, height: 72, backgroundColor: "#FF406E", marginLeft: 25 }}
    />
    <div style={{ display: "flex", alignItems: "center", marginBottom: "1em" }}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={48}
        height={48}
        viewBox="0 0 48 48"
        fill="none"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M41.46 14.24L41.18 13.74C40.8188 13.1354 40.3094 12.6329 39.7 12.28L26.28 4.54C25.6724 4.1875 24.9826 4.00124 24.28 4H23.7C22.9974 4.00124 22.3076 4.1875 21.7 4.54L8.28 12.3C7.67394 12.6505 7.17052 13.1539 6.82 13.76L6.54 14.26C6.1875 14.8677 6.00124 15.5575 6 16.26V31.76C6.00124 32.4626 6.1875 33.1524 6.54 33.76L6.82 34.26C7.17958 34.859 7.68098 35.3604 8.28 35.72L21.72 43.46C22.3246 43.8198 23.0164 44.0066 23.72 44H24.28C24.9826 43.9988 25.6724 43.8126 26.28 43.46L39.7 35.7C40.312 35.3574 40.8174 34.852 41.16 34.24L41.46 33.74C41.8082 33.1306 41.9942 32.442 42 31.74V16.24C41.9988 15.5375 41.8126 14.8477 41.46 14.24ZM23.7 8H24.28L36 14.76L24 21.68L12 14.76L23.7 8ZM26 39L37.7 32.24L38 31.74V18.22L26 25.16V39Z"
          fill="#FF406E"
        />
      </svg>
      <div style={{ marginLeft: "1em" }}>
        <h3>Pay for the order and shipping</h3>
        <p>
          The final costs will be ready after the order confirmation and
          <br /> shipping estimation from the carrier provider
        </p>
      </div>
    </div>
    <div
      style={{ width: 2, height: 72, backgroundColor: "#FF406E", marginLeft: 25 }}
    />
    <div style={{ display: "flex", alignItems: "center", marginBottom: "1em" }}>
      {/* Иконка виджета (замените на вашу иконку) */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={48}
        height={48}
        viewBox="0 0 48 48"
        fill="none"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M41.46 14.24L41.18 13.74C40.8188 13.1354 40.3094 12.6329 39.7 12.28L26.28 4.54C25.6724 4.1875 24.9826 4.00124 24.28 4H23.7C22.9974 4.00124 22.3076 4.1875 21.7 4.54L8.28 12.3C7.67394 12.6505 7.17052 13.1539 6.82 13.76L6.54 14.26C6.1875 14.8677 6.00124 15.5575 6 16.26V31.76C6.00124 32.4626 6.1875 33.1524 6.54 33.76L6.82 34.26C7.17958 34.859 7.68098 35.3604 8.28 35.72L21.72 43.46C22.3246 43.8198 23.0164 44.0066 23.72 44H24.28C24.9826 43.9988 25.6724 43.8126 26.28 43.46L39.7 35.7C40.312 35.3574 40.8174 34.852 41.16 34.24L41.46 33.74C41.8082 33.1306 41.9942 32.442 42 31.74V16.24C41.9988 15.5375 41.8126 14.8477 41.46 14.24ZM23.7 8H24.28L36 14.76L24 21.68L12 14.76L23.7 8ZM26 39L37.7 32.24L38 31.74V18.22L26 25.16V39Z"
          fill="#FF406E"
        />
      </svg>
      {/* Заголовок и описание */}
      <div style={{ marginLeft: "1em" }}>
        <h3>Wait for the delivery</h3>
        <p>
          The order will be delivered to your door by a delivery man of our
          cargo-partners
        </p>
      </div>
    </div>
    <div
      style={{ width: 2, height: 72, backgroundColor: "#FF406E", marginLeft: 25 }}
    />
    <div style={{ display: "flex", alignItems: "center", marginBottom: "1em" }}>
      {/* Иконка виджета (замените на вашу иконку) */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={48}
        height={48}
        viewBox="0 0 48 48"
        fill="none"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M41.46 14.24L41.18 13.74C40.8188 13.1354 40.3094 12.6329 39.7 12.28L26.28 4.54C25.6724 4.1875 24.9826 4.00124 24.28 4H23.7C22.9974 4.00124 22.3076 4.1875 21.7 4.54L8.28 12.3C7.67394 12.6505 7.17052 13.1539 6.82 13.76L6.54 14.26C6.1875 14.8677 6.00124 15.5575 6 16.26V31.76C6.00124 32.4626 6.1875 33.1524 6.54 33.76L6.82 34.26C7.17958 34.859 7.68098 35.3604 8.28 35.72L21.72 43.46C22.3246 43.8198 23.0164 44.0066 23.72 44H24.28C24.9826 43.9988 25.6724 43.8126 26.28 43.46L39.7 35.7C40.312 35.3574 40.8174 34.852 41.16 34.24L41.46 33.74C41.8082 33.1306 41.9942 32.442 42 31.74V16.24C41.9988 15.5375 41.8126 14.8477 41.46 14.24ZM23.7 8H24.28L36 14.76L24 21.68L12 14.76L23.7 8ZM26 39L37.7 32.24L38 31.74V18.22L26 25.16V39Z"
          fill="#FF406E"
        />
      </svg>
      {/* Заголовок и описание */}
      <div style={{ marginLeft: "1em" }}>
        <h3>Get your cosmetic!</h3>
        <p>Get your order and enjoy!</p>
      </div>
    </div>
    <div
      className="container"
      style={{
        display: "flex",
        maxWidth:
          "100%" /* максимальная ширина контейнера  равная ширине экрана */,
        padding: "2cm",
        alignItems: "center",
        gap: 80,
        backgroundSize: "cover",
        backgroundColor: "#F6F6F8"
      }}
    ></div>
  </>
  

  );
}
